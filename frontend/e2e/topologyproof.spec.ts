...
test("real desktop analysis flow", async ({ page }) => {
 const errors:string[]=[]; page.on("response", r=>{}); page.on("pageerror", e=>errors.push(e.message)); page.on("console", m=>{if(m.type()==="error" && !m.text().includes("404")) errors.push(m.text());});
