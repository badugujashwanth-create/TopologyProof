...
def create_analysis(request: AnalysisRequest, container=Depends(get_container))  # noqa: B008 -> dict[str,str]:
...
@router.get("/{run_id}")
def get_status(run_id: str, container=Depends(get_container))  # noqa: B008 -> dict[str,str]:
...
def findings(run_id: str, container=Depends(get_container))  # noqa: B008 -> object:
...
def report(run_id: str, container=Depends(get_container))  # noqa: B008 -> str:
